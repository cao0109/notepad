...
-  用于**分布式**、**协作式**和**超媒体信息系统**的应用层协议;
-  基于 **TCP/IP** 通信协议来传递数据的;

 URL 格式; **http://** 主机地址:端口/api     默认使用端口 **80** 

### **HTTP 三点注意事项：** 

- 无连接：它限制每次连接只处理一个请求，服务器处理完客户的请求，并收到客户的应答后，即断开连接，节省传输时间。
- 媒体独立：只要客户端和服务器知道如何处理的数据内容，任何类型的数据都可以通过HTTP发送，客户端以及服务器指定使用适合的 **MIME-type** 内容类型。
- 无状态：无状态是指协议对于事务处理没有记忆能力，缺少状态意味着如果后续处理需要前面的信息，则它必须重传，这样可能导致每次连接传送的数据量增大。相对而言，在服务器不需要先前信息时它的应答就较快。

### HTTP 协议通信流程： 

![cgiarch](HTTP.assets/cgiarch.gif)
...
HTTPS 在 通过 HTTP 进行通信的基础上采用 **SSL/TLS** 来加密数据包;

 URL 格式; **https://** 主机地址:端口/api		 默认使用端口**443** 

**作用**：提供对网站服务器的身份认证，保护交换资料的隐私与完整性。 

![img](HTTP.assets/1_5J6ULfBAvgLF8PBM4B__Qw.jpeg)

# HTTP 消息结构


基于客户端/服务端（C/S）的架构模型，通过一个可靠的链接来交换信息，是一个无状态的请求/响应协议。 
...
浏览器显示的内容有 HTML、XML、GIF、Flash 等，浏览器是通过 MIME Type 区分它们，决定用什么内容什么形式来显示。
...
### [MIME 对照表](https://www.runoob.com/http/mime-types.html)